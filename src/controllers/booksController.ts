...
export const createBook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const addBook = await client.query(
      'INSERT INTO book(title, published_year) VALUES ($1, $2) RETURNING *',
    ...
    for (const author of authorIds) {
      ...
      if (checkAuthor.rows.length === 0) {
        ...
        res.status(404).json({
          message: `Author with id ${author} not found, create book again`,
        });
        return;
      }
      await client.query(
    ...
    res
      .status(201)
      .json({ message: 'Book created successfully', book: addBook.rows[0] });
  } catch (error) {
    console.log(error);
    await client.query('ROLLBACK');
...
export const updateBook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updatedbook = await client.query(
      'UPDATE book SET title = $1, published_year = $2 WHERE book_id = $3',
    ...
    for (const author of authorIds) {
      ...
      if (checkAuthor.rows.length === 0) {
        await client.query('ROLLBACK');
        res.status(404).json({
          error: `Author not found id : ${author}, please update again with correct author ids`,
        });
        return;
      }
      await client.query(
...
export const deleteBook = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const book_auth_rel = await pool.query(
