...
export const getAuthors = async(req : Request, res : Response) => {
    try {
        ...
        if(authors.rowCount === 0){
            res.status(404).json({message : "Authors do not exist yet, add them first"});
        }
...
export const getAuthorById = async(req : Request, res : Response) => {
    try {
        const { id } = req.params;
        const author = await pool.query(`SELECT * FROM author WHERE author_id = $1`, [id]);
...
export const createAuthor = async(req : Request, res : Response) => {
    try {
        ...
        res.status(201).json({message : "new Author successfully added" ,author : addAuthor.rows[0]});
    } catch (error) {
        console.log(error);
...
export const updateAuthor = async(req : Request, res : Response) => {
    try {
        const {id} = req.params;
        const {name, bio} = req.body;
        const updatedAuthor = await pool.query("UPDATE author SET name = $1, bio = $2 WHERE author_id = $3", [name, bio, id]);
        if(updatedAuthor.rowCount === 0){
            res.status(404).json({message : "Author not found, enter correct author ID"});
            return;
        }
        res.status(201).json({
